...
/*
  General rules to set the accuracy.
...
const long min_accuracy = 1024;//ok this is a bit overkilling perhaps
...
struct prob_dist_data{
  ...
  */
  ...
  std::vector<NTL::ZZ> nb_internal_nodes;
...
void get_DDG_tree(struct prob_dist_data & T)
{
  ...
  */
  ...
  T.nb_internal_nodes.push_back(NTL::ZZ(2));//(there are always 2 nodes for the 1st level indexed by (1 - 1) )
  bool chk_accuracy_not_reached=false;
  for(long l=1;l<min_accuracy;l++) 
	...
	{
	  tmp = tmp + (NTL::ZZ(1)<<(l-m))*NTL::ZZ((T.L[m].size()));
	}
      
      //std::cout << "\nl = " << l << "    *   " << (NTL::ZZ(1)<<(l+1))-tmp;
      T.nb_internal_nodes.push_back((NTL::ZZ(1)<<(l+1))-tmp);
      if(( (NTL::ZZ(1)<<(l+1))-tmp == 0) )
	{
	  //std::cout << "\nOK\n";
	  chk_accuracy_not_reached=true;
	  break;
	}
    }
  if(!chk_accuracy_not_reached)
    {
      //May or may not be reached.
      //Even if reached, almost impossible to be required by the generating algorithm.
      std::cout << "\nValidate accuracy required.\nAccuracy is " << min_accuracy << " bits.\n";
    }
