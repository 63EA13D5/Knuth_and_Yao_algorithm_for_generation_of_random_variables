void simple_checks(struct prob_dist_data T)
{
  ...
  std::cout << "\n\nSupport size is " << T.nb_atoms << ".";
  std::cout << "\n\nA sample will be generated.\nYou will be queried for its size."
	    << "\n\nThe average number of random bits required to generate a random outcome will be printed."
	    << "\n\nThe average time for the generation of a random variable will be displayed."
	    << "\n\nEmpirical frequencies aside with theoritical probabilities will be displayed.\nYou are being queried for the number comparisons.\nA number between 1 and " << T.nb_atoms << ".\nKeep the sample size in mind (to be asked after).\nHow many of comparisons should be printed out? ";
  ...
  long index_rv;
  long five_percent = (long)(floor(0.05*(double)sample_size));
  for(long t = 0 ; t < sample_size; t++)
    {
      ...
      if((t%five_percent)==0){std::cout << "Sample generation: " << 5*(t/five_percent) << " % complete.\n";}
      counts[index_rv]++;
    }
  std::cout << "Sample generation: " << 100 << " % complete.\n";
...
void query_user(struct prob_dist_data & T)
{
  ...
  if(std::stol(choice) == 1)
      ...
      if(dis<=0)
	{
      ...
      do
	{
	  NTL::RR x = NTL::power( (NTL::conv<NTL::RR>(z)-alpha)/beta , 2);
	  NTL::RR y = NTL::exp(-x);
